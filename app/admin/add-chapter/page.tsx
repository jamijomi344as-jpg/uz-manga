...
export default function AddChapterPage() {
  ...
  const handleAddChapter = async (e: React.FormEvent) => {
    ...
    // XATO TUZATILDI: manga_id va pdf_url to'g'ri formatda yuborildi
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white p-6 md:p-12 font-sans">
      <div className="max-w-2xl mx-auto bg-[#141414] border border-zinc-800 p-8 rounded-2xl shadow-2xl">
        <div className="flex items-center gap-3 mb-8 border-b border-zinc-800 pb-4">
          <div className="bg-[#6E3FD1] p-3 rounded-xl text-white shadow-lg shadow-purple-500/20"><BookOpen size={24} /></div>
          <div>
        ...
        <form onSubmit={handleAddChapter} className="space-y-6">
          <div>
            ...
            <select value={selectedMangaId} onChange={(e) => setSelectedMangaId(e.target.value)} className="w-full bg-[#0a0a0a] border border-zinc-800 text-white rounded-xl p-4 outline-none cursor-pointer focus:border-[#6E3FD1] transition">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="number" step="any" value={chapterNumber} onChange={(e) => setChapterNumber(e.target.value)} placeholder="Masalan: 65" className="w-full bg-[#0a0a0a] border border-zinc-800 text-white rounded-xl p-4 focus:border-[#6E3FD1] outline-none transition" />
            </div>
            <div>
              <label className="block text-xs font-bold text-zinc-500 uppercase tracking-widest mb-2">Fayl Manzili (PDF URL)</label>
              <input type="text" value={fileUrl} onChange={(e) => setFileUrl(e.target.value)} placeholder="https://.../bob-65.pdf" className="w-full bg-[#0a0a0a] border border-zinc-800 text-white rounded-xl p-4 focus:border-[#6E3FD1] outline-none transition" />
            </div>
          </div>
          <button type="submit" disabled={loading || !selectedMangaId || !chapterNumber || !fileUrl} className="w-full mt-8 bg-[#6E3FD1] hover:bg-[#8356E8] disabled:bg-zinc-800 disabled:text-zinc-500 text-white font-black text-lg py-4 rounded-xl shadow-[0_0_30px_rgba(110,63,209,0.3)] transition-all flex justify-center items-center gap-2 active:scale-95">
