...
export default function BookmarksPage() {
  // Sahifa yuklanayotganini tekshirish (Hydration error oldini olish uchun)
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Xatcho'plarni yuklashda xatolik:", err);
    }
  }, []);

  // Xatcho'plardan o'chirish funksiyasi
  const removeBookmark = (id: any, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  // Agar sahifa hali to'liq yuklanmagan bo'lsa (SSR), bo'sh joy qaytaramiz
  if (!isMounted) return <div className="min-h-screen bg-[#0E0E10]"></div>;
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 sm:gap-6">
            {bookmarks.map((manga) => (
              <div key={manga.id} className="relative group">
                <Link href={`/manga/${manga.id}`} className="flex flex-col cursor-pointer">
                ...
                {/* O'chirish tugmasi - Link tashqarisida bo'lishi kerak */}
                <button
                  onClick={(e) => removeBookmark(manga.id, e)}
                  className="absolute top-2 right-2 bg-black/70 p-2 rounded-lg text-gray-400 hover:text-red-500 hover:bg-black transition z-20 backdrop-blur-md"
