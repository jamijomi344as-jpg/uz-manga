  ...
  return (
    <div className="min-h-screen bg-[#0E0E10] text-white font-sans">
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-10">
        <Link href="/" className="inline-flex items-center gap-2 text-gray-400 hover:text-white mb-8 transition"><ArrowLeft size={18}/> Asosiyga qaytish</Link>
        ...
        {bookmarks.length > 0 ? (
            ...
            {bookmarks.map((manga) => (
              <Link href={`/manga/${manga.id}`} key={manga.id} className="group relative flex flex-col cursor-pointer">
                <div className="relative aspect-[3/4] w-full overflow-hidden rounded-xl mb-3 bg-[#1A1A1D] border border-gray-800 group-hover:border-purple-500/50 transition-all">
                  ...
                  <button onClick={(e) => removeBookmark(manga.id, e)} className="absolute top-2 right-2 bg-black/70 p-2 rounded-lg text-gray-400 hover:text-red-500 hover:bg-black transition z-10 backdrop-blur-md">
                    <Trash2 size={16} />
                  </button>
                </div>
                <h2 className="text-[14px] font-bold text-gray-200 line-clamp-2 leading-tight group-hover:text-purple-400">{manga.title}</h2>
          ...
          <div className="flex flex-col items-center justify-center py-20 bg-[#151518] rounded-2xl border border-gray-800/60">
            ...
            <Link href="/catalog" className="mt-6 bg-purple-600 hover:bg-purple-500 text-white px-6 py-2.5 rounded-xl font-bold transition-all shadow-[0_0_20px_rgba(147,51,234,0.3)]">Katalogga o'tish</Link>
          </div>
        )}
      </div>
    </div>
