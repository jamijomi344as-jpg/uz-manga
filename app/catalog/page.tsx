...
export default function CatalogPage() {
  ...
  async function loadMangas(query = '') {
    ...
    if (query.trim()) request = request.ilike('title', `%${query}%`);
  ...
  useEffect(() => {
    const timer = setTimeout(() => loadMangas(searchQuery), 400);
    ...
    <main className="min-h-screen bg-[#0E0E10] text-gray-200 font-sans">
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-10">
        <div className="relative w-full max-w-2xl mx-auto mb-12">
          ...
          <input type="text" placeholder="Manga qidirish..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full h-14 bg-[#151518] border border-gray-800 rounded-2xl pl-12 pr-12 focus:border-purple-600 outline-none text-white" />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
            {mangas.map((manga) => (
              <Link href={`/manga/${manga.id}`} key={manga.id} className="group flex flex-col">
                ...
                <h3 className="text-sm font-bold group-hover:text-purple-500 truncate">{manga.title}</h3>
              </Link>
            ))}
          </div>
        ) : (
